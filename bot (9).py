...
async def remux_to_streaming_mp4(input_path: str) -> str:
    ...
    This is a fast "stream copy" (no re-encoding), so it takes seconds even
    for multi-GB files. If ffmpeg isn't available or every attempt fails,
    the original file is returned unchanged as a best-effort fallback.
    """
    ...
    attempts = [
        # 1) Straight remux, keep every stream as-is.
        ["ffmpeg", "-y", "-i", input_path, "-map", "0", "-c", "copy",
         "-movflags", "+faststart", output_path],
        # 2) Fallback: some MKV rips carry embedded subtitle/attachment
        #    streams (e.g. ASS subs, fonts) that an MP4 container can't
        #    hold — keep only video + audio.
        ["ffmpeg", "-y", "-i", input_path, "-map", "0:v", "-map", "0:a",
