...
async def get_video_codec(input_path: str):
    try:
        ...
        print(f"ffprobe codec check failed for {input_path}: {e}")
        return None


async def get_audio_codec(input_path: str):
    try:
        proc = await asyncio.create_subprocess_exec(
        ...
        print(f"ffprobe audio codec check failed for {input_path}: {e}")
        return None
...
async def remux_to_streaming_mp4(input_path: str) -> str:
    ...
    codec, audio_codec = await asyncio.gather(
        get_video_codec(input_path), get_audio_codec(input_path)
    )

    needs_video_reencode = codec not in (None, "h264", "avc1")
    if needs_video_reencode:
        print(f"Video codec `{codec}` for {input_path} isn't browser-safe (likely HEVC) — re-encoding to H.264.")
